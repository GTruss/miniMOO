...
if (!player.programmer)
  player:notify("You need to be a programmer to edit code.");
elseif (!args)
  player:notify("Usage: @edit <object>:<verb>");
else
  $verb_editor:invoke(argstr, verb);
endif
```

## Verb: @forked/@forked-v*erbose

```yaml
names: ["@forked", "@forked-v*erbose"]
dobj: any
prep: any
iobj: any
owner: "#0"
...
for task in (tasks)
  {q_id, start, unused, ticks, owner, vloc, vname, lineno, task_this, size} = task;
  if (player.wizard || owner == player)
    if (!shown)
      player:notify("Queue ID    Starts In   Owner         Verb (Line) [This]");
      player:notify("--------    ---------   -----         ------------------");
      shown = 1;
    endif

    starts = start > now ? tostr(start - now, "s") | "ready";
    owner_name = valid(owner) ? owner.name | tostr("Dead ", owner);
    player:notify(tostr(
      $string_utils:left(tostr(q_id), 10),
      "  ",
      $string_utils:left(starts, 10),
      "  ",
      $string_utils:left(owner_name, 12),
      "  ",
      vloc,
      ":",
      vname,
      ...
      lineno,
      ")",
      task_this != vloc ? tostr(" [", task_this, "]") | ""));
  endif
endfor
