...
public static partial class WorldSeeder {
    ...
    private static void AddPlayerPrototype(InMemoryObjectRepository repo) {
        ...
        genPlayer.Verbs.Add(ScriptVerb(["linelen"], """
            return abs(this.linelen);
        """, VerbObjectSpec.This, "none", VerbObjectSpec.This));

        genPlayer.Verbs.Add(ScriptVerb(["tell_contents"], """
            c = args[1];
            if (c)
              longear = {};
              gear = {};
    ...
    private static void AddProgrammer(InMemoryObjectRepository repo) {
        ...
        Prop(genProg, "prog_options", new MooValue.List([]));
        ...
        genProg.Verbs.Add(ScriptVerb(["prog_option"], """
            return $prog_options:get(this.prog_options, args[1]);
        """, VerbObjectSpec.Any));

        genProg.Verbs.Add(ScriptVerb(["@list", "@list#"], """
            bynumber = verb == "@list#";
    ...
    private static void AddProgrammerOptionsPrototype(InMemoryObjectRepository repo) {
        var genProgrammerOptions = Obj(WorldIds.ProgrammerOptions, ObjectId.System, WorldIds.GenericOptionPackage, null, "$prog_options");
        genProgrammerOptions.Flags = ObjectFlags.Readable | ObjectFlags.Fertile;

        repo.Add(genProgrammerOptions);
    }
